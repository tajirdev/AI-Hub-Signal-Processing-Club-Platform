...
class ResourceService:
    ...
    def create_resource(db: Session, resource: ResourceCreate, uploaded_by: int):
        ...
        type_val = resource.type.value if hasattr(resource.type, "value") else str(resource.type)
        db_resource = Resource(
            ...
            subgroup_id=resource.subgroup_id,
    ...
    def get_all_resources(db: Session, page: int = 1, limit: int = 10, search: str = None, subgroup_id: int = None, resource_type: str = None, sort: str = "created_at", sort_order: str = "desc"):
        query = db.query(Resource).options(
            joinedload(Resource.file),
            joinedload(Resource.subgroup)
        )
    ...
    def update_resource(resource_id: int, request: ResourceUpdate, db: Session, current_user: Users):
        ...
        if resource.uploaded_by != current_user.id and "super_admin" not in roles and "editor" not in roles:
        ...
        if request.type is not None:
            resource.type = request.type.value if hasattr(request.type, "value") else str(request.type)
        if request.external_url is not None:
            resource.external_url = str(request.external_url) if request.external_url else None
...
class ResourceMediaService:
    ...
    def CreateFile(resource_id: int, path: str, mime_type: str, original_filename: str, db: Session, current_user_id: int):
        ...
        if resource.uploaded_by != current_user_id and "super_admin" not in roles and "editor" not in roles:
            raise HTTPException(status_code=403, detail="Not authorized")

        media_record = db.query(Media).filter(Media.id == resource.file_id).first() if resource.file_id else None
