...
def blog_post_create(data:BlogPostCreate,current_user,db:Session):
...
def get_all_blog_post(current_user,db:Session,
...
def get_blog_post_by_id(post_id:int,current_user,db:Session):
    roles=[ur.Roles.name for ur in current_user.userRole]
...
def update_blog_post(post_id:int,data:BlogPostUpdate,current_user,db:Session):
    roles=[ur.Roles.name for ur in current_user.userRole]
    post=db.query(BlogPost).filter(BlogPost.id==post_id).first()
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="blog post not found")
...
def delete_blog_post(post_id:int,current_user,db:Session):
    roles=[ur.Roles.name for ur in current_user.userRole]
    post=db.query(BlogPost).filter(BlogPost.id==post_id).first()
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="blog post not found")
