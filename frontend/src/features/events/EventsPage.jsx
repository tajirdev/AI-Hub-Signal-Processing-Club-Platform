...
export function EventsPage() {
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      ...
      setError("Failed to load event papers. Please try again later.");
    } finally {
      setLoading(false);
    }
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6">
          ...
          <ScrollReveal animation="fade-up" delay={100}>
            <h1 className="text-5xl md:text-6xl lg:text-7xl font-heading font-black text-navy dark:text-white leading-tight tracking-tight mb-6">
              Our <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber to-blue-600 dark:from-amber dark:to-orange-500">Events</span>
            </h1>
          </ScrollReveal>
        ...
        <div className="max-w-[1280px] mx-auto">
          ...
          <div className="mb-12 flex flex-col md:flex-row gap-4 items-center justify-between">
            <ScrollReveal animation="fade-right" delay={100} className="w-full md:w-[400px]">
              <div className="relative w-full">
                ...
                <input 
                  type="text" 
                  placeholder="Search event papers..." 
                  value={searchTerm}
            ...
            <ScrollReveal animation="fade-left" delay={100}>
              <div className="text-sm text-gray-500 dark:text-gray-400 font-medium">
                Showing {eventList.length} of {totalItems} paper{totalItems !== 1 ? 's' : ''}
              </div>
            </ScrollReveal>
          </div>
          ...
          {loading ? (
            ...
            <EmptyState 
              ...
              message={debouncedSearch ? "No papers match your current search." : "No event papers have been published yet."} 
            />
          ) : (
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
                {eventList.map((event, index) => (
                  <ScrollReveal key={event.id} animation="fade-up" delay={(index % 9) * 100}>
                    <EventCard event={event} className="w-full sm:w-full md:w-full" />
                  </ScrollReveal>
                ))}
              </div>
