...
import { fetchNews } from '../../services/endpoints';
...
export function NewsPage() {
  ...
  const loadNews = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await fetchNews({ page, limit: 10, search: debouncedSearch || undefined });
      // Handle the case where the API might return an array directly or a paginated object
      const items = Array.isArray(data) ? data : (data.news || data.newsItems || data.results || data.items || []);
      setNewsList(items);
      setTotalPages(data.total_pages || 1);
      setTotalItems(data.total || items.length);
    } catch (err) {
      console.error("Failed to load newsItem:", err);
      setError("Failed to load newsItem papers. Please try again later.");
    } finally {
      setLoading(false);
    }
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6">
          ...
          <ScrollReveal animation="fade-up" delay={200}>
            <p className="text-lg md:text-xl text-gray-600 dark:text-gray-400 font-medium leading-relaxed max-w-2xl mx-auto">
              Stay updated with our recent press releases, achievements, and club updates.
            </p>
          </ScrollReveal>
        </div>
      </section>
      ...
      <section className="flex-1 py-16 px-6 relative z-10">
        <div className="max-w-[1280px] mx-auto">
          ...
          <div className="mb-12 flex flex-col md:flex-row gap-4 items-center justify-between">
            <ScrollReveal animation="fade-right" delay={100} className="w-full md:w-[400px]">
              <div className="relative w-full">
                ...
                <input 
                  type="text" 
                  placeholder="Search newsItem papers..." 
                  value={searchTerm}
            ...
            <ScrollReveal animation="fade-left" delay={100}>
              <div className="text-sm text-gray-500 dark:text-gray-400 font-medium">
                Showing {newsList.length} of {totalItems} paper{totalItems !== 1 ? 's' : ''}
              </div>
            </ScrollReveal>
          </div>
          ...
          {loading ? (
            ...
            <EmptyState 
              title="No news found" 
              message={debouncedSearch ? "No papers match your current search." : "No newsItem papers have been published yet."} 
            />
          ) : (
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
                {newsList.map((newsItem, index) => (
                  <ScrollReveal key={newsItem.id} animation="fade-up" delay={(index % 9) * 100}>
                    <NewsCard news={newsItem} className="w-full sm:w-full md:w-full" />
                  </ScrollReveal>
                ))}
              </div>
