...
import { fetchBlogPosts } from '../../services/endpoints';
...
export function BlogPage() {
  const [postList, setBlogList] = useState([]);
  ...
  const loadBlogPosts = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await fetchBlogPosts({ page, limit: 10, search: debouncedSearch || undefined });
      // Handle the case where the API might return an array directly or a paginated object
      const items = Array.isArray(data) ? data : (data.posts || data.results || data.items || []);
      setBlogList(items);
      setTotalPages(data.total_pages || 1);
      setTotalItems(data.total || items.length);
    } catch (err) {
      console.error("Failed to load post:", err);
      setError("Failed to load post papers. Please try again later.");
    } finally {
      setLoading(false);
    }
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6">
          ...
          <ScrollReveal animation="fade-up" delay={100}>
            <h1 className="text-5xl md:text-6xl lg:text-7xl font-heading font-black text-navy dark:text-white leading-tight tracking-tight mb-6">
              Our <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber to-blue-600 dark:from-amber dark:to-orange-500">Blog</span>
            </h1>
          </ScrollReveal>
      ...
      <section className="flex-1 py-16 px-6 relative z-10">
        <div className="max-w-[1280px] mx-auto">
          ...
          <div className="mb-12 flex flex-col md:flex-row gap-4 items-center justify-between">
            <ScrollReveal animation="fade-right" delay={100} className="w-full md:w-[400px]">
              <div className="relative w-full">
                ...
                <input 
                  type="text" 
                  placeholder="Search post papers..." 
                  value={searchTerm}
            ...
            <ScrollReveal animation="fade-left" delay={100}>
              <div className="text-sm text-gray-500 dark:text-gray-400 font-medium">
                Showing {postList.length} of {totalItems} paper{totalItems !== 1 ? 's' : ''}
              </div>
            </ScrollReveal>
          </div>
          ...
          {loading ? (
            ...
            <ErrorState message={error} onRetry={loadBlogPosts} />
          ) : postList.length === 0 ? (
            <EmptyState 
              title="No articles found" 
              message={debouncedSearch ? "No papers match your current search." : "No post papers have been published yet."} 
            />
          ) : (
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
                {postList.map((post, index) => (
                  <ScrollReveal key={post.id} animation="fade-up" delay={(index % 9) * 100}>
                    <BlogCard post={post} className="w-full sm:w-full md:w-full" />
                  </ScrollReveal>
                ))}
              </div>
